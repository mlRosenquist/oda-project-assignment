...
if __name__ == '__main__':
    ...
    start = time.time()
    model_raw = Classifier.perceptron_mse_classify(mnist_dataSet_raw, 0.002, 'adaptive')
    stop = time.time()
    print(f"Training time for {classifierName} 2d: {stop - start}s", file=logfile)

    start = time.time()
    model_2d = Classifier.perceptron_mse_classify(mnist_dataSet_2d, 0.1, 'invscaling')
    stop = time.time()
    print(f"Training time for {classifierName} 2d: {stop - start}s", file=logfile)
